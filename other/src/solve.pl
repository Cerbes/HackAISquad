...
auto_solve(N) :-
        ...
        new_var_struct(Vi),
        new_chs(CHSi),
        %once(prime_chs(CHSi, CHS2, Vi, V1)),
        %force(write('as\n')),
        solve_goals(Q2, Vi, Vo, CHSi, CHSo, [], _), % find an answer set
...
%! solve_goals(+Goals:list, +VarsIn:compound, -VarsOut:compound, +CHSin:list, -CHSout:list, +CallStack:list, -EvenLoops:list)
...
solve_goals([X | T], Vi, Vo, CHSi, CHSo, Cs, E) :-
        if_debug(1, (
        ...
        %force(write('sgs\n')),
        solve_goal(X, Vi, V1, CHSi, CHS1, Cs, E1),
        ...
        solve_goals(T, V1, Vo, CHS1, CHSo, Cs, E2),
        append(E1, E2, E).
solve_goals([], V, V, C, C, _, []). % no goals left; inductive success

%! solve_goal(+Goal:compound, +VarsIn:compound, -VarsOut:compound, +CHSin:list, -CHSout:list, +CallStack:list, -EvenLoops:list)
...
%        loops fail.
% @param EvenLoops List of even loops involving parent goal.
solve_goal(G, Vi, Vo, CHSi, CHSo, Cs, E) :-
        G = forall(V, G2), % forall
        !,
        solve_forall(V, G2, Vi, Vo, CHSi, CHSo, Cs, E).
solve_goal(G, V, V, CHS, CHS, _, []) :-
        G = builtin_1(G2), % Call to builtin
        !,
        format_term(G2, G3, _, V), % fill in variables, strip prefixes, etc.
        call(G3).
solve_goal(G, Vi, Vo, CHS, CHS, _, []) :-
        G =.. [O | _],
        operator(O, _, _),
        !,
...
solve_goal(G, Vi, Vo, CHSi, CHSo, Cs, E) :-
        ...
        !,
        solve_predicate(G, Vi, Vo, CHSi, CHSo, Cs, E).
solve_goal(G, Vi, Vo, CHSi, CHSo, Cs, E) :-
        ...
        %force(write('sg\n')),
        solve_predicate(G, Vi, Vo, CHSi, CHSo, Cs, E).
solve_goal(G, _, _, _, _, _, _) :-
        write_error('encountered invalid goal %w', [G]),
        !,
        fail.

%! solve_predicate(+Goal:compound, +VarsIn:compound, -VarsOut:compound, +CHSin:list, -CHSout:list, +CallStack:list, -EvenLoops:list)
...
solve_predicate(G, Vi, Vo, CHSi, CHSo, Cs, E) :-
        ...
        expand_call(Cflag, Go, V1, Vo, CHSi, CHSo, Cs, El, E).
solve_predicate(G, Vi, Vo, CHSi, CHSo, Cs, E) :-
        G \= not(_),
        !,
        predicate(G, F, A),
        ...
        Cflag >= 0, % If we have coinductive failure, we can fail and backtrack immediately
        expand_call(Cflag, G, V1, Vo, CHSi, CHSo, Cs, El, E).

%! expand_call(+CHSflag:int, +Goal:compound, +VarsIn:compound, -VarsOut:compound, +CHSin:list, -CHSout:list, +CallStack:list, +EvenLoopIn:compound, -EvenLoopOut:list)
% Solve a single goal that we know is a predicate. Check the CHS first, then
% act based on the result.
%
...
expand_call(1, _, Vi, Vo, CHS, CHS, _, Ei, Eo) :- % coinductive success
        Ei = -(L, Lv),
        (Lv = [] ->
                Eo = [],
                Vo = Vi
        ...
        !.
expand_call(0, G, Vi, Vo, CHSi, CHSo, Cs, _, Eo) :- % not present
        predicate(G, F, A),
        if_debug(4, (
        ...
        once(add_to_chs(F, A, 0, E, [], Vi, V1, CHSi, CHS1)),
        if_debug(4, (
        ...
        expand_call2(G, Rs, V1, V2, CHS1, CHS2, [G | Cs], E1), % expand
        remove_from_chs(E, CHS2, CHS3), % remove original goal from CHS
        get_sub_vars(E1, Ev, V2, V3),
        once(add_to_chs(F, A, 1, _, Ev, V3, Vo, CHS3, CHSo)), % add succeeding goal to CHS
...
%! expand_call2(+Goal:compound, +Rules:list, +VarsIn:compound, -VarsOut:compound, +CHSin:list, -CHSout:list, +CallStack:list, -EvenLoops:list)
...
%        loops fail.
% @param EvenLoops List of even loops involving parent goal.
expand_call2(G, [X | _], Vi, Vo, CHSi, CHSo, Cs, E) :- % match
        rule(X, H, B),
        once(get_unique_vars(H, H2, B, B2, Vi, V1)),
        if_debug(3, (
                format_term(G, G2, C, V1),
                format_term(H2, H3, _, V1),
                writef('trying to unify %w (', [G2]),
                print_var_constraints(C),
                writef(') with %w\n', [H3])
                )),
        solve_unify(G, H2, V1, V2, 0), % unify goal and head args
        solve_goals(B2, V2, Vo, CHSi, CHSo, Cs, E).
expand_call2(G, [_ | T], Vi, Vo, CHSi, CHSo, Cs, E) :- % not a match or backtracking
        if_debug(2, (
                format_term(G, G2, C, Vi),
        ...
        !,
        expand_call2(G, T, Vi, Vo, CHSi, CHSo, Cs, E).
...
%! solve_forall(+Var:ground, +Goal:compound, +OuterVars:list, +VarsIn:compound, -VarsOut:compound, +CHSin:list, -CHSout:list, +CallStack:list, -EvenLoops:list)
...
% @param OuterVars Variables from outer foralls. After calling the goal, fail if
%       any are bound.
% @param VarsIn Input var struct.
% @param VarsOut Output var struct.
% @param CHSin Input CHS.
...
%        loops fail.
% @param EvenLoops List of even loops involving parent goal.
solve_forall(V, G, Vi, Vo, Ci, Co, Cs, E) :-
        ...
        solve_goals([G], V1, V2, Ci, C1, Cs, E1), % solve once here
        %fill_in_variable_values(G, Gp, [], _, V1),
        %force(writef('post forall solve_goals: %w\n', [Gp])),
        %force(chs:print_chs(C1, V1, 2)),
        solve_forall2(V, Old, G, V2, V3, C1, C2, Cs, E2), % pass original variable values
        ...
        once(add_to_chs(F, A, 1, _, Ev, V5, Vo, C2, Co)), % add succeeding goal to CHS, but don't fail if already there
        if_debug(4, (
                format_term(G2, G3, _, Vo),
                writef('FORALL CHS marked success via forall for %w\n', [G3])
                )).
        
%! solve_forall2(+Var:list, +OldValue, +Goal:compound, +VarsIn:compound, -VarsOut:compound, +CHSin:list, -CHSout:list, +CallStack:list, -EvenLoops:list)
...
%        loops fail.
% @param EvenLoops List of even loops involving parent goal.
solve_forall2(Var, _, _, V, V, C, C, _, []) :- % var is unbound; we're done
        is_unbound(Var, V, [], _, _),
        if_debug(4, writef('solve forall: var %w is unbound\n', [Var])),
        !.
solve_forall2(Var, Ov, G, Vi, Vo, Ci, Co, Cs, E) :-
        ...
        solve_forall3(Var, Val, G, Vi, Vo, Ci, Co, Cs, E).
solve_forall2(Var, Ov, G, Vi, Vo, Ci, Co, Cs, E) :-
        is_unbound(Var, Vi, Val, _, _),
        Val = [_ | _],
        ...
        solve_forall3(Var, Val2, G, Vi, Vo, Ci, Co, Cs, E).
solve_forall2(Var, _, _, V, _, _, _, _, _) :-
        var_value(Var, V, Val),
        if_debug(4, (
...
%! solve_forall3(+Var:ground, +Values:list, +Goals:list, +VarsIn:compound, -VarsOut:compound, +CHSin:list, -CHSout:list, +CallStack:list, -EvenLoops:list) is nondet
...
%        loops fail.
% @param EvenLoops List of even loops involving parent goal.
solve_forall3(Var, [X | T], G, Vi, Vo, Ci, Co, Cs, E) :-
        substitute(Var, X, G, G2), % substitute value for variable
        if_debug(4, (
                format_term(G2, G3, _, Vi),
                format_term(X, X2, _, Vi),
                writef('FORALL: calling %w with %w = %w\n', [G3, Var, X2])
                )),
        %force(writef('forall3 solve_goals: %w\n', [G2])),
        solve_goals([G2], Vi, V1, Ci, C1, Cs, E1), % call with ground value
        %force(writef('back in forall3, T = %w, chs = %w\n', [T, C1])),
        solve_forall3(Var, T, G, V1, Vo, C1, Co, Cs, E2),
        append(E1, E2, E).
solve_forall3(_, [], _, V, V, C, C, _, []).
