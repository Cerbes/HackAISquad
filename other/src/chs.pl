...
%! chs_entry(+Entry:compound, ?Functor:ground, ?Args:list, ?Success:int) is det.
%! chs_entry(?Entry:compound, +Functor:ground, +Args:list, +Success:int) is det.
...
%%! prime_chs(+CHSin:compound, -CHSout:compound, +VarsIn:list, -VarsOut:compound) is det
% Load facts into the CHS.
%
% @param CHSin Input CHS.
% @param CHSout Output CHS.
% @param VarsIn Input variables.
% @param VarsOut Output variables.
%prime_chs(Ci, Co, Vi, Vo) :-
%        findall(H, defined_rule(_, H, []), F), % get facts
%        !,
%        prime_chs2(F, Ci, Co, Vi, Vo).
        
%%! prime_chs2(+Facts:list, +CHSin:compound, -CHSout:compound, +VarsIn:compound, -VarsOut:compound) is det
% Load each fact in a list into the CHS as succeeding.
%
...
% @param VarsIn Input variables.
% @param VarsOut Output variables.
%prime_chs2([X | T], Ci, Co, Vi, Vo) :-
%        X =.. [F | A],
%        add_to_chs(F, A, 1, _, [], Vi, V1, Ci, C1),
%        !,
%        prime_chs2(T, C1, Co, V1, Vo).
%prime_chs2([], C, C, V, V).
...
get_dual_entries(E, Ens, C, V) :-
        ...
        chs_entry(En, Fn, _, 1), % dummy entry for negation
        once(findall(An, (member(En, Ens2), chs_entry(En, _, An, _)), Ens3)), % get args for entries matching negated functor
...
exact_match(E, [X | _], V, S) :-
        chs_entry(E, F, A1, _),
        chs_entry(X, F, A2, S), % functors match
...
%! add_to_chs(+Functor:ground, +Args:list, +Success:compound, -Entry: compound, +SubVars:list, +VarsIn:compound, -VarsOut:compound, +CHSin:compound, -CHSout:compound) is det
...
% @param CHSin Input CHS.
% @param CHSout Output CHS.
add_to_chs(F, A, S, E, Ev, Vi, Vo, CHSi, CHSo) :- % not already present
        ...
        %writef('chs added %w\n', [E]),
        !.
add_to_chs(F, A, S, E, _, V, V, CHS, CHS) :- % already present; skip
        chs_entry(E, F, A, S).%,
