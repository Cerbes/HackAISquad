...
public class Query{
    public static void main(String[] args) throws Exception {
        ...
        do {// run as long as the user wants to  look up snakes
            ...
            GoodWinRedirect.call(1);

            snakes = ReadNames.call(); //this if reading prolog's output

            printSaps2(snakes);

            GoodWinRedirect.call(2);

            ReadVenomous.call(); //getting if venomous from output
    ...
    public static void printSaps2(String[] snakes) throws FileNotFoundException{
        String filename;
        filename = "query.lp"; //create filename to open files with
        File file = new File(filename); //create file object
        try(PrintWriter output = new PrintWriter(file);){ //create scanner object in try block to that it closes the file automatically
